...
def create_calendar_event(event: CalendarEventCreate, db: Session = Depends(get_db)):
    status = "Completed" if event.end_date and event.end_date < datetime.utcnow() else "Scheduled"
...
def update_calendar_event(event_id: int, updated_event: CalendarEventCreate, db: Session = Depends(get_db)):
    ...
    event.status = "Completed" if updated_event.end_date and updated_event.end_date < datetime.utcnow() else "Scheduled"
